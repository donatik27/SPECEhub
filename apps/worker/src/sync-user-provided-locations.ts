...
// User's curated list with locations
...
async function syncUserProvidedLocations() {
  console.log('🗺️  Syncing user-provided trader locations...\n')
  
  // First, load Twitter usernames from ALL periods
  console.log('📥 Step 1: Loading Twitter usernames from Polymarket API...')
  
  const allTraders = new Map<string, any>() // twitterUsername -> trader data
  ...
  for (const period of ['day', 'week', 'month', 'all']) {
    ...
    while (offset < 1000) {
      ...
      data.forEach(t => {
        if (t.xUsername && !allTraders.has(t.xUsername)) {
          allTraders.set(t.xUsername, t)
        }
      })
  ...
  console.log(`✅ Found ${allTraders.size} unique traders with Twitter from API\n`)
  
  // Step 2: Sync to database
  console.log('📥 Step 2: Syncing to database...')
  let synced = 0
  
  for (const [xUsername, traderData] of allTraders) {
    try {
  ...
  console.log(`✅ Synced ${synced} traders to database\n`)
  
  // Step 3: Update geoloc ations for user's list
  console.log('🗺️  Step 3: Updating geolocations for your list...\n')
  
  let updated = 0
  let notFound = 0
  
  for (const [twitterUsername, location] of Object.entries(USER_LOCATIONS)) {
    // Fuzzy match Twitter username
    const normalizedSearch = normalize(twitterUsername)
    ...
    const trader = await prisma.trader.findFirst({
      ...
      const allTradersDb = await prisma.trader.findMany({
        where: {
      ...
      return allTradersDb.find(t => {
        const normalizedDb = normalize(t.twitterUsername!)
    ...
    if (!trader) {
      console.log(`   ⚠️  Not found: @${twitterUsername}`)
      notFound++
      continue
    }
    ...
    console.log(`   ✅ @${twitterUsername.padEnd(30)} (${trader.displayName}) → ${coords.country}`)
    updated++
  }
  
  console.log(`\n📊 Summary:`)
  console.log(`   ✅ Updated: ${updated}/${Object.keys(USER_LOCATIONS).length}`)
  console.log(`   ⚠️  Not found: ${notFound}`)
  console.log('\n✅ Complete!')
}

syncUserProvidedLocations()
  .then(() => process.exit(0))
  .catch(error => {
